    ...
    with ui.card().classes("w-full mb-4"):
        ...
        async def load_scripts():
            ...
            for s in scripts:
                ...
                label = f"[ID:{s[0]}] {s[1]} ({s[2]}){img_badge} – {s[3]}..."
                options[s[0]] = label
                    ...
                    with result_container:
                        with ui.card().classes(
                            ...
                            with ui.row().classes("gap-4 items-start"):
                                if result["success"]:
                                    ui.image(result["url"]).classes("w-80 rounded shadow-lg")
                                with ui.column().classes("flex-1"):
                                    status = "✅ 성공" if result["success"] else "❌ 실패"
                                    cached = " (캐시)" if result.get("cached") else ""
                                    ui.label(f"{item['scene_id']}: {status}{cached}").classes("font-semibold")
                                    ui.label(f"⏱️ {result['elapsed']:.1f}초").classes("text-sm text-gray-400")
                                    ui.label(result["prompt"][:120] + "...").classes("text-xs text-gray-500")

                success_count = sum(1 for r in state["results"] if r["success"])
                progress_bar.value = 1.0
                progress_label.text = f"🎉 완료! {success_count}/{total}개 이미지 → script_{state['script_id']}/ 폴더"
